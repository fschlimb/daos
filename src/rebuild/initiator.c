 ...
 */
static int
rebuild_one_queue(struct rebuild_iter_obj_arg *iter_arg,
...
static int
rebuild_enum_unpack_cb(struct dss_enum_unpack_io *io, void *arg)
{
...
rebuild_obj_ult(void *data)
{
	...
	if (arg->snaps) {
		...
		for (i = 0; i < arg->snap_cnt; i++) {
			/* Skip the object created after this snapshot */
			if (arg->epoch > arg->snaps[i])
				continue;

			epr.epr_lo = i > 0 ? arg->snaps[i-1] : 0;
	...
	epr.epr_lo = arg->snaps ? arg->snaps[arg->snap_cnt - 1] : 0;
