		    ...
		    struct dtx_batched_cleanup_blob *bcb, umem_off_t *next_dsb)
{
	...
	/* The last blob for active DTX entries, keep and reset it. */
	if (cont_df->cd_dtx_active_head == cont_df->cd_dtx_active_tail) {
		D_ASSERT(dtx_is_null(dsb->dsb_prev));
		D_ASSERT(dtx_is_null(dsb->dsb_next));

		/* dsb_index is just next to dsb_count */
		...
		bcb->bcb_dae_count = 0;
		memset(bcb->bcb_recs, 0, sizeof(umem_off_t) * dsb->dsb_cap);

		if (next_dsb != NULL)
			*next_dsb = bcb->bcb_dsb_off;
	...
	if (cont_df->cd_dtx_active_tail == bcb->bcb_dsb_off) {
		...
		D_ASSERT(!dtx_is_null(cont_df->cd_dtx_active_tail));
	}
...
static int
vos_dtx_create_act_table(struct vos_container *cont)
{
	struct umem_instance		*umm = vos_cont2umm(cont);
	struct vos_cont_df		*cont_df = cont->vc_cont_df;
	struct dtx_batched_cleanup_blob	*bcb;
	...
	/* cd_dtx_active_tail is just next to cd_dtx_active_head */
	umem_tx_add_ptr(umm, &cont_df->cd_dtx_active_head,
			sizeof(cont_df->cd_dtx_active_head) +
			sizeof(cont_df->cd_dtx_active_tail));
...
static int
vos_dtx_create_cmt_table(struct umem_instance *umm, struct vos_cont_df *cont_df)
{
	struct vos_dtx_scm_blob		*dsb;
	umem_off_t			 dsb_off;
	...
	dsb_off = umem_zalloc(umm, DTX_SCM_BLOB_SIZE);
	if (dtx_is_null(dsb_off)) {
		D_ERROR("No space when create committed DTX table.\n");
		return -DER_NOSPACE;
	}
	...
	/* cd_dtx_committed_tail is just next to cd_dtx_committed_head */
...
do_dtx_rec_release(struct umem_instance *umm, struct vos_container *cont,
		   ...
		   struct vos_dtx_record_df *rec_df, int index,
		   bool abort, bool destroy, bool *sync)
{
		...
		struct dtx_batched_cleanup_blob **bcb_p)
{
	...
	/* For abort or non-destroy case, cleanup DTX entry synchronously. */
	if (abort || !destroy)
		sync = true;
...
vos_dtx_commit_one(struct vos_container *cont, struct dtx_id *dti,
		   daos_epoch_t epoch, struct vos_dtx_cmt_ent_df *dce_df)
{
	struct umem_instance		*umm = vos_cont2umm(cont);
	...
	memcpy(dce_df, &dce->dce_base, sizeof(*dce_df));
...
vos_dtx_alloc(struct umem_instance *umm, struct dtx_handle *dth)
{
	...
	if (dtx_is_null(cont_df->cd_dtx_active_tail)) {
		rc = vos_dtx_create_act_table(cont);
		if (rc != 0)
			D_GOTO(out, rc);
	}
	...
	if (dsb->dsb_index >= dsb->dsb_cap) {
		struct vos_dtx_scm_blob		*dsb_prev = dsb;

		dsb_off = umem_zalloc(umm, DTX_SCM_BLOB_SIZE);
		if (dtx_is_null(dsb_off)) {
			D_ERROR("No space to store actvie DTX (2) "DF_DTI"\n",
				DP_DTI(&DAE_XID(dae)));
		...
		dsb = umem_off2ptr(umm, dsb_off);
		dsb->dsb_magic = DTX_ACT_BLOB_MAGIC;
		dsb->dsb_cap = (DTX_SCM_BLOB_SIZE -
				sizeof(struct vos_dtx_scm_blob)) /
			       sizeof(struct vos_dtx_act_ent_df);
...
vos_dtx_prepared(struct dtx_handle *dth)
{
	...
	if (dth->dth_solo) {
		...
		if (dae->dae_records != NULL) {
			...
			umem_tx_add_ptr(umm, rec_df, size);
			memcpy(rec_df, dae->dae_records, size);
			DAE_REC_OFF(dae) = rec_off;
		}

		DAE_INDEX(dae) = dsb->dsb_index;
		dae_df = umem_off2ptr(vos_cont2umm(cont), dae->dae_df_off);

		/* For non-new allocated @dsb */
		if (dsb->dsb_count != 0) {
			umem_tx_add_ptr(umm, dae_df, sizeof(*dae_df));
			/* dsb_index is just next to dsb_count */
			umem_tx_add_ptr(umm, &dsb->dsb_count,
					sizeof(dsb->dsb_count) +
					sizeof(dsb->dsb_index));
			...
			int count, daos_epoch_t epoch)
{
	...
	if (dtx_is_null(cont_df->cd_dtx_committed_tail)) {
		rc = vos_dtx_create_cmt_table(umm, cont_df);
		if (rc != 0)
			return rc;
	}
	...
	if (slots > 0) {
		...
		for (i = 0; i < slots; i++) {
			rc = vos_dtx_commit_one(cont, &dtis[i], epoch,
						&dce_df[i]);
			if (rc1 == 0)
				rc1 = rc;
		}
		...
		if (dce_df != &dsb->dsb_commmitted_data[dsb->dsb_count]) {
			memcpy(&dsb->dsb_commmitted_data[dsb->dsb_count],
			       dce_df, sizeof(*dce_df) * slots);
			D_FREE(dce_df);
		}
	...
	umem_tx_add_ptr(umm, &dsb_prev->dsb_next, sizeof(dsb_prev->dsb_next));
	dsb_prev->dsb_next = dsb_off;
	...
	for (i = 0; i < count; i++) {
		rc = vos_dtx_commit_one(cont, &dtis[i + slots], epoch,
					&dce_df[i]);
		if (rc1 == 0)
			rc1 = rc;
	}
...
vos_dtx_aggregate(daos_handle_t coh)
{
	...
	if (cont_df->cd_dtx_committed_head ==
	    cont_df->cd_dtx_committed_tail) {
		/* The last blob for committed DTX entries, keep and reset it */
		D_ASSERT(dtx_is_null(dsb->dsb_prev));
		D_ASSERT(dtx_is_null(dsb->dsb_next));
