		 ...
		 d_iov_t *iovs, uint32_t iov_nr)
{
	...
	if (tgt_head->esh_first == OBJ_EC_SEG_NIL) {
		...
		if (tmp_iov->iov_buf + tmp_iov->iov_len == iovs[0].iov_buf) {
		 ...
		 bool add_parity)
{
	...
	uint64_t	start, end, r_start, r_end, tmp_idx, tmp_nr;
	uint32_t	i, first_tgt, end_tgt, tgt;
	...
	end_tgt = obj_ec_tgt_of_recx_idx(end - 1, stripe_rec_nr,
					   obj_ec_cell_rec_nr(oca));
	for (i = 0; i < obj_ec_data_tgt_nr(oca); i++) {
		if (i < first_tgt)
			...
			r_start = roundup(start, cell_rec_nr) +
				  (i - first_tgt - 1) * cell_rec_nr;

		if (i < end_tgt)
			r_end = rounddown(end, stripe_rec_nr) + i * cell_rec_nr;
		else if (i == end_tgt)
			r_end = end;
		else
			r_end = rounddown(end, stripe_rec_nr) - stripe_rec_nr +
				(i + 1) * cell_rec_nr;
		D_ASSERT(r_end > r_start);
		D_ASSERT(i == obj_ec_tgt_of_recx_idx(r_start, stripe_rec_nr,
						     cell_rec_nr));
		tmp_idx = ec_vos_idx(r_start);
		tmp_nr = ec_vos_idx(r_end) - tmp_idx;
		  ...
		  struct obj_reasb_req *reasb_req, uint32_t iod_idx)
{
	...
	for (i = 0, idx = 0; i < obj_ec_tgt_nr(oca); i++) {
		if (tgt_recx_nrs[i] != 0)
			tidx[i] = idx++;
		else
			tidx[i] = -1;
	...
	for (i = 0, rec_nr = 0, last = 0; i < obj_ec_tgt_nr(oca); i++) {
		...
		siod->siod_off = rec_nr * cell_bytes;
